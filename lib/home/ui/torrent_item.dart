...
String? _compactDetail(TorrentInfoResponse torrent, AppLocalizations l10n) {
  if (_isCompleted(torrent.state)) {
    final ratio = torrent.ratio;
    if (ratio == null || ratio < 0) return null;
    return formatRatio(ratio);
  }
  final eta = torrent.eta;
  if (eta == null || eta < 0 || eta >= 8640000) return null;
  return formatEta(eta, l10n);
}
...
class _StateLabel extends StatelessWidget {
  const _StateLabel({required this.state, this.detail});
  ...
  Widget build(BuildContext context) {
    ...
    final label = resolved.label(context.l10n);
...
class SpeedChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final textTheme = Theme.of(context).textTheme;
    final style = (compact ? textTheme.labelLarge : textTheme.bodyMedium)
        ?.copyWith(
          color: color,
