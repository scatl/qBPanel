...
`torrents`                    | object  | Property: torrent hash, value: same as [torrent list](#get-torrent-list) (full `serialize_torrent` object, including fields the official Wiki omits such as `total_wasted`)
...
The response is a JSON array. Each item is produced by `serialize_torrent` and is **the same shape** as `sync/maindata.torrents[hash]`. Official Wiki table is incomplete vs source; fields below follow `src/webui/api/serialize/serialize_torrent.h`. Some keys (e.g. `total_wasted`, `pieces_*`, `connections_*`) appear on newer 5.x builds; older 5.0.x omit them.
...
`connections_count`               | integer | Number of current connections
`connections_limit`               | integer | Connection limit for this torrent
...
`created_by`                      | string  | Creator string from torrent metadata
`creation_date`                   | integer | Torrent creation time (Unix Epoch)
`dl_limit`                        | integer | Torrent download speed limit (bytes/s). `-1` if unlimited.
`dlspeed`                         | integer | Torrent download speed (bytes/s)
`download_path`                   | string  | Incomplete download path (empty if unused)
...
`has_metadata`                    | bool    | True if torrent metadata is available
`hash`                            | string  | Torrent ID / info-hash used as map key in maindata
`infohash_v1`                     | string  | v1 infohash (empty if hybrid/v2-only without v1)
`infohash_v2`                     | string  | v2 infohash (empty if v1-only)
`inactive_seeding_time_limit`     | integer | Per-torrent inactive seeding time limit (seconds). `-2` = use global; `-1` = no limit
...
`max_inactive_seeding_time`       | integer | Effective inactive seeding time limit (seconds)
...
`pieces_have`                     | integer | Number of pieces already downloaded
`pieces_num`                      | integer | Total number of pieces
`popularity`                      | float   | Torrent popularity
`priority`                        | integer | Torrent priority. Returns -1 if queuing is disabled or torrent is in seed mode
`private`                         | bool    | True if torrent is from a private tracker. **Actual list/maindata JSON key.** Wiki historically listed this as `isPrivate` (that key is used by `/torrents/properties`, not by `serialize()`)
`progress`                        | float   | Torrent progress (percentage/100)
`ratio`                           | float   | Torrent share ratio. Max ratio value: 9999.
`ratio_limit`                     | float   | Per-torrent share ratio limit (vs effective `max_ratio`)
`reannounce`                      | integer | Time until the next tracker reannounce
`root_path`                       | string  | Absolute root path of torrent content
...
`seeding_time_limit`              | integer | Per-torrent seeding time limit (seconds). When Automatic Torrent Management is disabled, `max_seeding_time` is set to this value. If Automatic Torrent Management is enabled, the value is -2. Unset default is -1.
...
`share_limit_action`              | string  | Action when share limits are reached (enum string; later 5.x)
`share_limits_mode`               | string  | Share-limits mode (enum string; later 5.x)
...
`total_wasted`                    | integer | Total data wasted for torrent (bytes): failed hashes, duplicate pieces, etc.
`tracker`                         | string  | The first tracker with working status. Returns empty string if no tracker is working.
`trackers_count`                  | integer | Number of trackers
...
`forcedMetaDL`| Torrent is forced to fetch metadata and ignore queue limit
